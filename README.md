...
> python ./P2PApplication.py 2000 test_user_1

\<portNumber\> refers to the port the user should connect to. It must be at least 2000 and in intervals of 100
...
### Example:
Peer 1 on port 2100 wants the file "text.txt"

It contacts the tracker (port 1025) using port 2100, and finds that Peers 2 (2200), 3 (2300), 4 (2400), and 5 (2500) have it

Peer 1 then starts a thread to handle the download using port 2101\
From here on out, Peer 1 doesn't use the main port 2100 for anything, instead using the additional ports (2101 - 2199)\
This frees the main port 2100 to handle other tasks

Peer 1 contacts Peer 2 (2200) using port 2101 to get the length of the file, which is 100 bytes\
Note: Peer 2 doesn't repond with it's main port (2200), instead it will respond using one of the additional ports\
...
- Peer 1 contacts Peer 2 (2200) using port 2102, requesting bytes 1-25 of the file\
- Peer 1 contacts Peer 3 (2300) using port 2103, requesting bytes 26-50 of the file\
- Peer 1 contacts Peer 4 (2400) using port 2104, requesting bytes 51-75 of the file\
- Peer 1 contacts Peer 5 (2500) using port 2105, requesting bytes 76-199 of the file
...
It then contacts the Tracker (1025) using port 2106, informing the Tracker that it has the "text.txt" file
